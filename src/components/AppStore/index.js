...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, inputVal: ''}
  ...
  getTabAppsList = () => {
    const {activeTabId, inputVal} = this.state
    const filteredAppsList = appsList.filter(
      eachApp =>
        eachApp.category === activeTabId &&
        eachApp.appName.toLowerCase().includes(inputVal.toLowerCase()),
    )
  ...
  render() {
    const {activeTabId, inputVal} = this.state
    const filteredAppsList = this.getTabAppsList()
    return (
